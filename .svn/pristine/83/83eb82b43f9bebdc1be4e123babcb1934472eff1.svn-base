...
import com.cc.helperqq.task.LoginTask;
...
public class QQError {
    ...
    public static boolean validate(HelperQQService context, TaskEntry entry) {
        ...
        AccessibilityNodeInfo closeBtn;
...
//        }else
        ...
        if (Utils.findViewByTextMatch(context, "身份过期") != null) {
            txt = "你的帐户信息已过期";
            Utils.sleep(1000);
            isTrue = loginOper(context, entry);
            ...
            Utils.sleep(1000);
            isTrue = loginOper(context, entry);
        } else if ((validateBtn = Utils.findViewByText(context, "禁止")) != null) {
            txt = "禁止授权";
            Utils.clickCompone(validateBtn);
            Utils.sleep(3 * 1000L);
            ...
            isTrue = false;
            reportError(entry.getWx_sign(), "-4");
        } else if (Utils.findViewByTextMatch(context, "下线通知") != null) {
            txt = "当前账号在另一个设备上登陆";
    ...
    private static boolean loginOper(HelperQQService context, TaskEntry entry) {
    ...
    public static void loginPromptAcitvity(HelperQQService context) {
        LogUtils.logInfo("loginPromptAcitvity    ");
        String txt = "";
